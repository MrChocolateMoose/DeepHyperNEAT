...
REDUCED_DATASET_PATH = '../shared/reduced_mnist.pkl'
MNIST_DATASET_PATH = '../deep_learning/data/mnist.pkl.gz'

# How long should training and fine-tuning the DBN take per population member?
REQUESTED_DBN_TIME_MIN = 10
...
def reduce_dataset(name, dataset_pair):
    ...
    nparray_reduced_classifications = numpy.asarray(reduced_classifications, dtype=long)
    npmatrix_reduced_images = numpy.asmatrix(reduced_images)

    return (npmatrix_reduced_images, nparray_reduced_classifications)
...
def load_mnist_dataset():
    ...
    train_set_count, valid_set_count, test_set_count = (len(reduced_datasets[0][1]),
                                                        len(reduced_datasets[1][1]),
                                                        len(reduced_datasets[2][1]))
    ...
    assert train_set_count == len(train_set[1])
    assert valid_set_count == len(valid_set[1])
    assert test_set_count == len(test_set[1])
