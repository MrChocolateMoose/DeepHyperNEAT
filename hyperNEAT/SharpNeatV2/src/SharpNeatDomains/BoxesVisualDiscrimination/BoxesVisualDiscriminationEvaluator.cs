...
namespace SharpNeat.Domains.BoxesVisualDiscrimination
{
    ...
    public class BoxesVisualDiscriminationEvaluator : IPhenomeEvaluator<IBlackBox>
    {
        ...
        /// </summary>
        public static DeepBeliefNetworkBiaser.DeepBeliefNetworkBiasEvaluator dbnEvalulator = new DeepBeliefNetworkBiaser.DeepBeliefNetworkBiasEvaluator(); 
        ...
        private double RunTrial(IBlackBox box, TestCaseField testCaseField, int largeBoxRelativePos, out double activationRange)
        {
            ...
            double[] targetArray = new double[box.OutputSignalArray.Length];
            box.OutputSignalArray.CopyTo(targetArray, 0);
            if (targetArray.Distinct().Count() > 2)
            {
